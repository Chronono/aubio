...
aubio_source_t * new_aubio_source_raw(const aubio_source_raw_data_t* aubio_source_raw_data, uint_t samplerate, uint_t hop_size)
{
	...
	if (s != NULL)
	{
		...
		s->source = AUBIO_NEW(aubio_source_raw_data_t);
		if (s->source != NULL)
			*(aubio_source_raw_data_t*)s->source = *aubio_source_raw_data;
	}
